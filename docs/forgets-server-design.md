...
AI 不需要理解 Perry/Fastify 内部就能知道 native compatibility status。
...
源码审阅能看到 fastify 的 lowering 预扫描、codegen native dispatch table、runtime declarations 和 common dispatch；这证明 fastify 是 Perry native 编译路径里的重要目标。但这些仍不是 Fastify 插件生态契约，也不能替代 native HTTP behavior suite。
...
native fastify handler dispatch 的并发行为不能只从 declarations 推出，必须用并发请求测试确认。
```
...
公开并发语义由 forgets 定义，不把 Tokio、Fastify 或 Perry 内部调度模型暴露给业务代码：
...
native fastify concurrent requests baseline

//! Windows text-id → widget-handle registry for `setText(id, value)`
//! (Phase 2 v3.3).
//!
...
//! `widgets::text::set_text_str` to update the Win32 STATIC control via
//! `SetWindowTextW`.
//!
//! The registry is process-global (Mutex-guarded for thread-safety, though
//! all UI calls are expected on the main thread). Calls to `setText` for an
//! unregistered id are silently ignored — matches macOS / HarmonyOS semantics.
...
/// Cross-platform setText handler. Looks up `id` and updates the matching
/// widget's text via `widgets::text::set_text_str` → `SetWindowTextW`.
